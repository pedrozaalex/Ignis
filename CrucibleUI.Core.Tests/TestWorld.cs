...
using CrucibleEntityNode = CrucibleUI.Core.Tests.Ecs.EntityNode;
using EntityNode = CrucibleUI.Core.Tests.Ecs.EntityNode;
...
public class TestWorld : IDisposable
{
    ...
    {
        var node = new EntityNode(root);
        LayoutEngine.Compute<EntityNode, EntityStore, SubLayoutContext, Entity, EntityCache>(node, Cache, Store, ref _subLayout);
    }
