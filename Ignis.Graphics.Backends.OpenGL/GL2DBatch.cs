...
/// Self-contained: manages its own shader and GL state at flush time.
/// </summary>
internal sealed class GL2DBatch : IDisposable
{
    ...
    public GL2DBatch(GL gl)
    {
        unsafe
        {
            ...
            // Create our own shader so we're self-contained
            _shader = new GLShader(gl, DefaultShaders.Shader2DVertex, DefaultShaders.Shader2DFragment);
    ...
    /// Call this before adding quads when the projection changes.
    /// </summary>
    ...
    public unsafe void Flush()
    {
        ...
        // Set up complete GL state for 2D rendering - we own this state
        _gl.Enable(EnableCap.Blend);
        _gl.BlendFunc(BlendingFactor.SrcAlpha, BlendingFactor.OneMinusSrcAlpha);

        // Use Lequal and disable depth writes to allow overlapping 2D elements
        // This matches the behavior we previously had in EndPass
        _gl.Enable(EnableCap.DepthTest);
        _gl.DepthFunc(DepthFunction.Lequal);
        _gl.DepthMask(false);

        // Use our own shader with current projection
        _shader.Use();
        _shader.SetMat4("uProjection", _projection);
        ...
        {
            // Ensure no texture is bound if we're not using one
            _gl.ActiveTexture(TextureUnit.Texture0);
