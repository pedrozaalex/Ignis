namespace Ignis.Engine.UI
{
    ...
    public enum LayoutType { Row, Column, Grid }
    public enum PositionType { Relative, Absolute }
    public enum UnitKind { Pixels, Percentage, Stretch, Auto }
    ...
    public struct Units : IEquatable<Units>
    {
        ...
        public Units(UnitKind kind, float value) { Kind = kind; Value = value; }
        public static Units Pixels(float v) => new(UnitKind.Pixels, v);
        public static Units Percentage(float v) => new(UnitKind.Percentage, v);
        public static Units Stretch(float v) => new(UnitKind.Stretch, v);
    ...
    public struct Size { public float Main; public float Cross; }
    public struct Rect { public float PosX; public float PosY; public float Width; public float Height; }
    ...
    public static class LayoutEngine
    {
        ...
        private static Size Compute(object node, LayoutType parentLayoutType, float parentMain, float parentCross, ILayoutNode store, ILayoutCache cache)
        {
            ...
            float computedMain = main.Kind switch {
            ...
            float computedCross = cross.Kind switch {
            ...
            float minCross = parentLayoutType is LayoutType.Row or LayoutType.Grid ? minH : minW;
            float maxCross = parentLayoutType is LayoutType.Row or LayoutType.Grid ? maxH : maxW;
            ...
            var relativeChildren = visibleChildren.Where(c => store.GetPositionType(c) == PositionType.Relative).ToList();

            // Content Sizing
            if ((main.IsAuto || cross.IsAuto) && relativeChildren.Count == 0)
            {
                var pW = width.IsAuto ? (float?)null : (parentLayoutType is LayoutType.Row or LayoutType.Grid ? computedMain : computedCross);
                var pH = height.IsAuto ? (float?)null : (parentLayoutType is LayoutType.Column ? computedMain : computedCross);
                
                var content = store.MeasureContent(node, pW, pH);
                if (content.HasValue)
                {
                    if (main.IsAuto) computedMain = parentLayoutType is LayoutType.Row or LayoutType.Grid ? content.Value.width : content.Value.height;
                    if (cross.IsAuto) computedCross = parentLayoutType is LayoutType.Row or LayoutType.Grid ? content.Value.height : content.Value.width;
                }
            }
            ...
            {
                return LayoutGrid(node, computedMain, computedCross, store, cache);
            }

            // Determine available space for children
            float childParentMain = layoutType == parentLayoutType ? computedMain : computedCross;
            float childParentCross = layoutType == parentLayoutType ? computedCross : computedMain;

            // Padding
            float padLeft = store.GetPaddingLeft(node).ToPx(childParentMain, 0);
            float padRight = store.GetPaddingRight(node).ToPx(childParentMain, 0);
            float padTop = store.GetPaddingTop(node).ToPx(childParentCross, 0);
            float padBottom = store.GetPaddingBottom(node).ToPx(childParentCross, 0);

            // Borders
            float borderLeft = store.GetBorderLeft(node).ToPx(childParentMain, 0);
            float borderRight = store.GetBorderRight(node).ToPx(childParentMain, 0);
            float borderTop = store.GetBorderTop(node).ToPx(childParentCross, 0);
            float borderBottom = store.GetBorderBottom(node).ToPx(childParentCross, 0);

            float padMainBefore = layoutType == LayoutType.Row ? padLeft : padTop;
            float padMainAfter = layoutType == LayoutType.Row ? padRight : padBottom;
            float padCrossBefore = layoutType == LayoutType.Row ? padTop : padLeft;
            float padCrossAfter = layoutType == LayoutType.Row ? padBottom : padRight;

            float borderMainBefore = layoutType == LayoutType.Row ? borderLeft : borderTop;
            float borderMainAfter = layoutType == LayoutType.Row ? borderRight : borderBottom;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (cross.IsAuto)
                        {
                            ...
                            if (content.HasValue)
                            {
                                childComputedCross = layoutType == LayoutType.Row ? content.Value.height : content.Value.width;
                            }
                        }
                    }
                }
            ...
            {
                 float mainSum = childrenData.Sum(c => c.main + c.mainAfter);
                 float crossMax = childrenData.Max(c => c.cross);
                 ...
                 if (layoutType == parentLayoutType)
                 {
                     ...
                     if (main.IsAuto) computedMain = Math.Clamp(mainSum + padMainBefore + padMainAfter + borderMainBefore + borderMainAfter, minMain, maxMain);
                     if (cross.IsAuto) computedCross = Math.Clamp(crossMax + padCrossBefore + padCrossAfter + borderCrossBefore + borderCrossAfter, minCross, maxCross);
                 }
                 else
                 {
                     // Perpendicular layout: main maps to cross, cross maps to main
                     if (main.IsAuto) computedMain = Math.Clamp(crossMax + padCrossBefore + padCrossAfter + borderCrossBefore + borderCrossAfter, minMain, maxMain);
                     if (cross.IsAuto) computedCross = Math.Clamp(mainSum + padMainBefore + padMainAfter + borderMainBefore + borderMainAfter, minCross, maxCross);
                 }
                 
                 // Re-update parents
                 childParentMain = (layoutType == parentLayoutType ? computedMain : computedCross) - padMainBefore - padMainAfter - borderMainBefore - borderMainAfter;
                 childParentCross = (layoutType == parentLayoutType ? computedCross : computedMain) - padCrossBefore - padCrossAfter - borderCrossBefore - borderCrossAfter;
            }
            ...
            // Position Children
            float currentMainPos = padMainBefore + borderMainBefore;
            var alignment = store.GetAlignment(node);
            
            // Alignment Offset
            float mainSumFinal = childrenData.Sum(c => c.main + c.mainAfter);
            float extraMain = (childParentMain + padMainBefore + padMainAfter + borderMainBefore + borderMainAfter) - (mainSumFinal + padMainBefore + padMainAfter + borderMainBefore + borderMainAfter); // Roughly
            // Simpler:
            float contentMain = mainSumFinal;
            float freeMain = (childParentMain) - contentMain;
            
            if (freeMain > 0) {
                if (alignment is Alignment.Center or Alignment.TopCenter or Alignment.BottomCenter) currentMainPos += freeMain * 0.5f;
                else if (alignment is Alignment.Right or Alignment.TopRight or Alignment.BottomRight) currentMainPos += freeMain;
            ...
            {
                var d = childrenData[i];
                float crossPos = padCrossBefore + borderCrossBefore;
                
                // Cross alignment
                float freeCross = childParentCross - d.cross;
                if (freeCross > 0) {
                     // Simple mapping for demo
                     if (alignment == Alignment.Center || alignment == Alignment.Left || alignment == Alignment.Right) crossPos += freeCross * 0.5f;
                     if (alignment == Alignment.BottomCenter || alignment == Alignment.BottomLeft || alignment == Alignment.BottomRight) crossPos += freeCross;
                }

                float x, y, w, h;
                if (layoutType == LayoutType.Row) {
                    x = currentMainPos; y = crossPos; w = d.main; h = d.cross;
                } else {
            ...
            foreach(var child in visibleChildren.Where(c => store.GetPositionType(c) == PositionType.Absolute))
            {
                 ...
                 float cw = childW.IsStretch ? computedMain : childW.ToPx(computedMain, 0);
                 float ch = childH.IsStretch ? computedCross : childH.ToPx(computedCross, 0);
                 
                 // Recursive layout for absolute child content
                 var size = Compute(child, layoutType, cw, ch, store, cache);
                 cw = size.Main; ch = size.Cross;
                 
                 cw = Math.Clamp(cw, cMinW, cMaxW);
                 ch = Math.Clamp(ch, cMinH, cMaxH);
                 
                 float left = store.GetLeft(child).ToPx(computedMain, 0);
                 float top = store.GetTop(child).ToPx(computedCross, 0);
        ...
        private static Size LayoutGrid(object node, float width, float height, ILayoutNode store, ILayoutCache cache)
        {
            ...
            float padLeft = store.GetPaddingLeft(node).ToPx(width, 0);
            float padRight = store.GetPaddingRight(node).ToPx(width, 0);
            float padTop = store.GetPaddingTop(node).ToPx(height, 0);
            float padBottom = store.GetPaddingBottom(node).ToPx(height, 0);
            
            float borderLeft = store.GetBorderLeft(node).ToPx(width, 0);
            float borderRight = store.GetBorderRight(node).ToPx(width, 0);
            float borderTop = store.GetBorderTop(node).ToPx(height, 0);
            float borderBottom = store.GetBorderBottom(node).ToPx(height, 0);
            
            float availableWidth = width - padLeft - padRight - borderLeft - borderRight;
            float availableHeight = height - padTop - padBottom - borderTop - borderBottom;
            float offsetX = padLeft + borderLeft;
            float offsetY = padTop + borderTop;
            
            var colTracks = store.GetGridColumns(node);
            var rowTracks = store.GetGridRows(node);
            
            // Resolve tracks (simplified: px and stretch only)
            float[] colSizes = new float[colTracks.Count];
            float usedW = 0;
            float flexColSum = 0;
            for(int i=0; i<colTracks.Count; i++) {
                if(colTracks[i].IsStretch) flexColSum += colTracks[i].Value;
                else { colSizes[i] = colTracks[i].ToPx(availableWidth, 0); usedW += colSizes[i]; }
            }
            if(flexColSum > 0) {
                float free = Math.Max(0, availableWidth - usedW);
                for(int i=0; i<colTracks.Count; i++) 
                    if(colTracks[i].IsStretch) colSizes[i] = (colTracks[i].Value / flexColSum) * free;
            }
            ...
            for(int i=0; i<rowTracks.Count; i++) {
                if(rowTracks[i].IsStretch) flexRowSum += rowTracks[i].Value;
                else { rowSizes[i] = rowTracks[i].ToPx(availableHeight, 0); usedH += rowSizes[i]; }
            }
            if(flexRowSum > 0) {
                float free = Math.Max(0, availableHeight - usedH);
                for(int i=0; i<rowTracks.Count; i++) 
                    if(rowTracks[i].IsStretch) rowSizes[i] = (rowTracks[i].Value / flexRowSum) * free;
            }
            
            // Calculate positions
            float[] colPos = new float[colSizes.Length + 1];
            for(int i=0; i<colSizes.Length; i++) colPos[i+1] = colPos[i] + colSizes[i];
            
            float[] rowPos = new float[rowSizes.Length + 1];
            for(int i=0; i<rowSizes.Length; i++) rowPos[i+1] = rowPos[i] + rowSizes[i];

            // Layout Children
            foreach(var child in store.GetChildren(node).Where(c => store.IsVisible(c)))
            {
                int cStart = store.GetColumnStart(child);
                int cSpan = store.GetColumnSpan(child);
                int rStart = store.GetRowStart(child);
                int rSpan = store.GetRowSpan(child);
                
                // Safe indices
                cStart = Math.Clamp(cStart, 0, colSizes.Length - 1);
                rStart = Math.Clamp(rStart, 0, rowSizes.Length - 1);
                
                float cx = colPos[cStart];
                float cy = rowPos[rStart];
                float cw = colPos[Math.Min(cStart + cSpan, colPos.Length - 1)] - cx;
                float ch = rowPos[Math.Min(rStart + rSpan, rowPos.Length - 1)] - cy;
