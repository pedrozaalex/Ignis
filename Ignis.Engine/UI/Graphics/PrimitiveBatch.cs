...
public class PrimitiveBatch : IDisposable
{
    ...
    private void EnsureCapacity(int additionalVertices, int additionalIndices)
    {
        ...
        while (_vertexCount + additionalVertices > _vertices.Length) Array.Resize(ref _vertices, _vertices.Length * 2);

        // Grow index buffer if needed
        while (_indexCount + additionalIndices > _indices.Length) Array.Resize(ref _indices, _indices.Length * 2);
    }
    ...
    public void DrawBorder(Rectangle bounds, float thickness, Color color, float radius = 0f)
    {
        ...
        // Clamp corner radius
        var maxRadius = Math.Min(w, h) * 0.5f;
        ...
        var segments = 8;
        DrawArcOutline(new Vector2(x + radius, y + radius), radius, radius - thickness, color, segments, 180,
            270); // Top-left
        DrawArcOutline(new Vector2(x + w - radius, y + radius), radius, radius - thickness, color, segments, 270,
            360); // Top-right
        DrawArcOutline(new Vector2(x + w - radius, y + h - radius), radius, radius - thickness, color, segments, 0,
            90); // Bottom-right
        DrawArcOutline(new Vector2(x + radius, y + h - radius), radius, radius - thickness, color, segments, 90,
            180); // Bottom-left
    }
    ...
    /// </summary>
    private void DrawArcOutline(Vector2 center, float outerRadius, float innerRadius, Color color, int segments,
        float startAngle, float endAngle)
    {
        ...
        var startRad = MathHelper.ToRadians(startAngle);
        var endRad = MathHelper.ToRadians(endAngle);
        ...
        for (var i = 0; i <= segments; i++)
        {
            ...
            _vertices[_vertexCount++] =
                new VertexPositionColor(new Vector3(center + direction * outerRadius, 0), color);
            // Inner vertex
            _vertices[_vertexCount++] =
                new VertexPositionColor(new Vector3(center + direction * innerRadius, 0), color);
        }

        // Generate quad indices for each segment
        for (var i = 0; i < segments; i++)
        {
            var outerCurrent = baseVertex + i * 2;
            var innerCurrent = baseVertex + i * 2 + 1;
            var outerNext = baseVertex + (i + 1) * 2;
            var innerNext = baseVertex + (i + 1) * 2 + 1;
    ...
    public void DrawCircle(Vector2 center, float radius, Color color, int segments = 32)
    {
        ...
        for (var i = 0; i <= segments; i++)
        {
            var angle = (float)i / segments * MathHelper.TwoPi;
    ...
    public void DrawRoundedRectangle(Rectangle bounds, float cornerRadius, Color color)
    {
        ...
        // Clamp corner radius
        var maxRadius = Math.Min(bounds.Width, bounds.Height) * 0.5f;
    ...
    /// </summary>
    private void DrawCircleSegment(Vector2 center, float radius, Color color, int segments, float startAngle,
        float endAngle)
    {
        if (segments < 1) segments = 1;
